...
class Worker:
    ...
    def _run_worker(self, name):
        ...
        while not self.coord.should_stop():
            ...
            for local_step in itertools.count():
                ...
                # Compute action probabilities with online net
                action_probs = self.online_net.predict_actions_probs(self.sess, state[np.newaxis])
    ...
    def calculate_td_target(self, next_state, reward, done):
        next_state_value = self.target_net.predict_state_value(self.sess, next_state[np.newaxis])
        # Calculate TD target
        if not done:
